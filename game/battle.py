    ...
    # 아이템 선택시 아이템 목록 출력
        ...
        for i in range(0, 4):
            ...
            if GPD.items[i].NUM == 0:
                GPD.Ingame_font.font.draw(180, 120 - i * 30, GPD.items[i].name + ' ' + str(GPD.items[i].NUM) + '개', [105, 105, 105])
            else:
                GPD.Ingame_font.font.draw(180, 120 - i * 30, GPD.items[i].name + ' ' + str(GPD.items[i].NUM) + '개', [255, 255, 255])
        # 아이템 2차메뉴일때 대상선택
        if sel_menu_type == 2:
